...
/**
 * Ported from legacy {@code init/MineFantasyMaterials.java} — the authoritative tier/durability/
 * weight/AC stat tables for every MFR material. Replaces the placeholder stats previously used in
 * {@link ModToolMaterials}.
 * <p>
 * Leather materials pass {@code null} for their (absent) crafting ingredient instead of legacy's
 * {@code Ingredient.EMPTY} — that constant no longer exists, and its suggested replacement
 * {@code Ingredient.of()} actually throws in MC 26 ({@code Ingredient}'s constructor rejects an
 * empty {@code HolderSet}). {@code CustomMaterial}/{@code CustomMaterialRegistry.NONE} already
 * treat a {@code null} ingredient as "no in-world ingredient", so this is the correct substitute.
 */
public class ModMaterials {
	...
	public static void initLeatherMaterials() {
		CustomMaterialRegistry.addMaterial(new CustomMaterial(Names.LEATHER, CustomMaterialType.LEATHER_MATERIAL,
				null, new int[]{198, 92, 53}, 1.0F, 0.4F, 1.2F, 0.5F,
				0F, 0.1F, 0, MFRRarity.COMMON, 15,
				0, null, null, null, null, false));

		CustomMaterialRegistry.addMaterial(new CustomMaterial(Names.HARD_LEATHER, CustomMaterialType.LEATHER_MATERIAL,
				null, new int[]{154, 72, 41}, 1.5F, 0.8F, 1.0F, 0.5F,
				0F, 0.2F, 1, MFRRarity.UNCOMMON, 20,
		...
		CustomMaterialRegistry.addMaterial(new CustomMaterial(Names.MINOTAUR_SKIN, CustomMaterialType.LEATHER_MATERIAL,
				null, new int[]{118, 69, 48}, 2.0F, 1.5F, 0.8F, 1.0F,
				0F, 0.5F, 2, MFRRarity.RARE, 25,
		...
		CustomMaterialRegistry.addMaterial(new CustomMaterial(Names.DRAGON_SKIN, CustomMaterialType.LEATHER_MATERIAL,
				null, new int[]{154, 72, 41}, 3.0F, 2.0F, 1.0F, 1.2F,
				0F, 0.75F, 3, MFRRarity.EPIC, 30,
	...
	public static void initBaseMaterials() {
		...
		DRAGONSCALE = BaseMaterial.addMaterial("dragonscale", 3, 3000, -1, 5F, -1, 20, 1.20F, 85, MFRRarity.RARE);
		...
		ORNATE = BaseMaterial.addMaterial("ornate", -1, 300, 0, 0.0F, 30, 1.00F, 30, MFRRarity.UNCOMMON).setForgeStats(1, 1, 4F, 120, 150);
		TUNGSTEN = BaseMaterial.addMaterial("tungsten", 2, 600, 3, 4F, 5, 1.50F, 0, MFRRarity.UNCOMMON).setForgeStats(3, 3, 5.0F, 150, 300);
		...
		DRAGONFORGE = BaseMaterial.addMaterial("dragonforge", 4, 1500, 4, 5.0F, 14, 1.00F, 60, MFRRarity.UNCOMMON)
				.setForgeStats(4, 4, 8.0F, 250, 350).setResistances(100F, 0F); // lvl 60
		...
		ADAMANTIUM = BaseMaterial.addMaterial("adamantium", 6, 3000, 6, 8.0F, 10, 1.25F, 90, MFRRarity.UNCOMMON)
				.setForgeStats(6, 5, 9.0F, 300, 400).setResistances(35F, 0F); // lvl 90
		MITHRIL = BaseMaterial.addMaterial("mithril", 6, 2500, 6, 7.0F, 30, 0.50F, 90, MFRRarity.UNCOMMON)
				.setForgeStats(6, 5, 6.0F, 280, 400).setResistances(0F, 35F); // lvl 90

		// Masterwork Materials (6) Level 100
		IGNOTUMITE = BaseMaterial.addMaterial("ignotumite", 7, 1000, 7, 14.0F, 20, 2.00F, 100, MFRRarity.RARE)
				.setForgeStats(7, 5, 15.0F, 350, 400).setResistances(50F, 0F); // High damage, heavy, fire resist lvl 100
		MITHIUM = BaseMaterial.addMaterial("mithium", 7, 1000, 7, 10.0F, 40, 0.25F, 100, MFRRarity.RARE)
				.setForgeStats(7, 5, 15.0F, 330, 400).setResistances(0F, 50F); // Low damage, light, magic resist lvl 100
		ENDERFORGE = BaseMaterial.addMaterial("enderforge", 7, 1000, 7, 12.0F, 20, 1.00F, 100, MFRRarity.RARE)
				.setForgeStats(7, 5, 15.0F, 400, 450).setResistances(25F, 25F); // Middle lvl 100

		// Engineer Materials
		COGWORKS = BaseMaterial.addMaterial("cogworks", 4, 500, -1, 1.0F, -1, 10, 1.00F, 85).setForgeStats(3, 3, 2.5F, 120, 250);
		COMPOSITE_ALLOY = BaseMaterial.addMaterial("composite_alloy", 4, 1800, -1, 4.0F, -1, 10, 2.00F, 85)
